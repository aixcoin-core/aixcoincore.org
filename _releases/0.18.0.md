...
`/Applications/Bitcoin-Qt` (on Mac) or `aixcoind`/`aixcoin-qt` (on
Linux).
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://aixcoincore.org/en/contact/) the Bitcoin Core
...
`~aixcoin/.aixcoin`. This change makes Bitcoin Core more consistent with
...
- A new `aixcoin-wallet` tool is now distributed alongside Bitcoin
